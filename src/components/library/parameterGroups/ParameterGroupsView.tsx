...
import { useParameterGroupsList } from "@/api/library";

import { LibraryHeader } from "../LibraryHeader";
import { useServerPagination } from "../hooks/useServerPagination";
import { useDebouncedValue } from "../hooks/useDebouncedValue";

import { ParameterGroupsTable } from "./ParameterGroupsTable";
...
export function ParameterGroupsView() {
  ...
  const q = useParameterGroupsList(listInput);
    ...
    <div className="space-y-4">
      ...
      {!isLoading && !isError ? (
        <div className="bg-background rounded-lg border border-border overflow-hidden">
          <ParameterGroupsTable items={pageItems} />
      ...
      {createOpen ? (
        <ParameterGroupCreateModal onClose={() => setCreateOpen(false)} />
      ) : null}
    </div>
  );
}
