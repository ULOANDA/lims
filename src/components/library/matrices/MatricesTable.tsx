...
import { Eye, Pencil, Trash2, Filter, X, Check } from "lucide-react";
...
import {
  useMatricesFilter,
  type MatricesFilterFrom,
  type MatricesFilterOtherFilter,
} from "@/api/library";
...
type Props = {
  ...
  onOpenDetail: (matrixId: string) => void;
...
function buildOtherFilters(
  filters: ExcelFiltersState,
  excludeKey: FilterKey
): MatricesFilterOtherFilter[] {
...
function ExcelFilterPopover(props: ExcelFilterPopoverProps) {
  ...
  const apiOptions = useMemo((): Array<OptionWithCount<string> | OptionWithCount<number>> => {
    ...
    if (props.type === "string") {
      return data
        .map((x) => ({ value: x.filterValue, count: x.count }))
  ...
  const filteredOptions = useMemo(() => {
    ...
    if (props.type === "string") {
      return (apiOptions as OptionWithCount<string>[]).filter((o) =>
        o.value.toLowerCase().includes(qtext)
      );
    }
    return (apiOptions as OptionWithCount<number>[]).filter((o) =>
      String(o.value).includes(qtext)
    );
        ...
        <tbody className="divide-y divide-border">
          {items.map((m) => {
            ...
            return (
              <tr
                key={rowKey}
                onClick={() => onSelectRow(rowKey, m.matrixId)}
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-foreground">
                  {formatNumberVi(m.feeBeforeTax) ?? t("common.noData")}
                </td>
