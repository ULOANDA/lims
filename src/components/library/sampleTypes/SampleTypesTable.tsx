...
import {
  useSampleTypesFilter,
  type SampleTypesFilterFrom,
  type SampleTypesFilterOtherFilter,
} from "@/api/library";
...
function normalizeDisplayStyle(
  i18n: { language: string },
  raw: unknown
): { label: string; stableKey: string } {
...
function buildOtherFilters(
  filters: SampleTypesExcelFiltersState,
  excludeKey: FilterKey
): SampleTypesFilterOtherFilter[] {
    ...
    return data
      .map((x) => {
        if (props.filterKey === "displayTypeStyle") {
          const { label, stableKey } = normalizeDisplayStyle(
            i18n,
            x?.filterValue
          );
...
export function SampleTypesTable(props: Props) {
  const { t, i18n } = useTranslation();
  const { items, excelFilters, onExcelFiltersChange } = props;
