...
export function SampleDetailModal({ sample, receipt, onClose, onSave }: SampleDetailModalProps) {
  ...
  const [attachedFiles] = useState<Array<{
    fileId: string;
    fileName: string;
      ...
      <div className="fixed inset-4 bg-background rounded-lg shadow-xl z-50 flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              {t("reception.sampleDetail.title", { code: sample.sampleId })}
            </h2>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("reception.sampleDetail.description")}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-muted/30 p-4 rounded-lg border border-border">
              <h3 className="text-sm font-semibold text-foreground mb-3">
                {t("reception.sampleDetail.relatedReceipt")}
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <DraggableInfoTable
              title={t("reception.sampleDetail.productDetails")}
              data={productDetails}
              isEditing={isEditing}
              onChange={setProductDetails}
            />
            <DraggableInfoTable
              title={t("reception.sampleDetail.testingInfo")}
              data={testingInfo}
              isEditing={isEditing}
