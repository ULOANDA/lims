...
function isApiResponseShape(x: unknown): x is ApiResponse<unknown> {
  ...
  return (
    ...
    typeof (x as { statusCode?: unknown }).statusCode === "number"
  );
}
...
function sanitizeClientBase(raw: unknown): Omit<ClientDetail, "clientId" | "clientName"> & {
  entity: EntityInfo;
  clientId: string;
  clientName: string;
} | null {
...
export async function clientsGetList(input: {
  query: ClientsListQuery;
}): Promise<ApiResponse<ClientListItem[]>> {
  const raw: unknown = await api.getRaw<unknown, ClientsListQuery>(
    "/v2/clients/get/list",
    {
      query: input.query,
...
export async function clientsGetDetail(input: {
  query: { clientId: string };
}): Promise<ApiResponse<ClientDetail>> {
  const raw: unknown = await api.getRaw<unknown, { clientId: string }>(
    "/v2/clients/get/detail",
    {
      query: input.query,
      headers: { "Cache-Control": "no-cache", Pragma: "no-cache" },
    }
...
export async function clientsCreate(input: {
  body: ClientsCreateBody;
}): Promise<ApiResponse<ClientDetail>> {
  const res = await api.post<ClientDetail, ClientsCreateBody>(
    "/v2/clients/create",
    { body: input.body }
  );
...
export async function clientsUpdate(input: {
  body: ClientsUpdateBody;
}): Promise<ApiResponse<ClientDetail>> {
  const res = await api.post<ClientDetail, ClientsUpdateBody>(
    "/v2/clients/update",
    {
      body: input.body,
      headers: { "Cache-Control": "no-cache", Pragma: "no-cache" },
...
export async function clientsDelete(input: {
  body: ClientsDeleteBody;
}): Promise<ApiResponse<null | { clientId: string } | boolean>> {
  return api.post<null | { clientId: string } | boolean, ClientsDeleteBody>(
    "/v2/clients/delete",
    { body: input.body }
  );
}

export async function clientsFilter(input: {
  body: ClientsFilterBody;
}): Promise<ApiResponse<ClientListItem[]>> {
  const raw: unknown = await api.postRaw<unknown, ClientsFilterBody>(
    "/v2/clients/filter",
    {
      body: input.body,
      headers: { "Cache-Control": "no-cache", Pragma: "no-cache" },
...
export const clientsKeys = {
  ...
  detail: (clientId: string) =>
    [...clientsKeys.all, "detail", clientId] as const,

  allList: (input: { query: ClientsListQuery }) =>
    [...clientsKeys.all, "all", stableKey(input)] as const,

  filter: (input: { body: ClientsFilterBody }) =>
    [...clientsKeys.all, "filter", stableKey(input)] as const,
};

export function useClientsList(
  input: { query: ClientsListQuery },
...
export function useClientsDetail(
  input: { query: { clientId: string } },
  opts?: { enabled?: boolean }
) {
  return useQuery({
...
export function useClientsAll(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const totalPages =
        (isObject(meta) && typeof meta.totalPages === "number"
          ? meta.totalPages
          : 1) ?? 1;

      const all: ClientListItem[] = Array.isArray(firstRes.data)
        ? [...firstRes.data]
        : [];
