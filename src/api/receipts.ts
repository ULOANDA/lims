...
function assertSuccessWithMeta<T>(res: ApiResponse<T>): ListResult<T> {
  ...
  const meta = (res.meta ?? null) as ApiMeta | null;
...
export async function receiptsCreateFull(
  ...
  const { body, query } = input;
  return api.post<ReceiptDetail, ReceiptsCreateFullBody>(
    "/v2/receipts/create/full",
    {
      body,
      query,
    },
...
export type ReceiptsFilterFrom =
  | "receiptStatus"
  | "clientId"
  | "receiptCode"
...
export async function receiptsFilter(
  input: ReceiptsFilterInput,
): Promise<ApiResponse<ReceiptsFilterItem[]>> {
...
export const receiptsKeys = {
  ...
  list: (input?: ReceiptsGetListInput) =>
    [...receiptsKeys.all, "list", stableKey(input ?? {})] as const,

  full: (input: ReceiptsGetFullInput) =>
    [
      ...receiptsKeys.all,
      "full",
      input.receiptId,
      stableKey(input.query ?? {}),
    ] as const,

  filter: (input: ReceiptsFilterInput) =>
    [...receiptsKeys.all, "filter", stableKey(input)] as const,
};
...
export function useReceiptFull(
  input: ReceiptsGetFullInput,
  opts?: { enabled?: boolean },
) {
  return useQuery({
...
export function useReceiptsFilter(
  input: ReceiptsFilterInput,
  opts?: { enabled?: boolean },
) {
  return useQuery({
...
export function useCreateReceipt() {
  ...
  return useMutation({
    mutationFn: async (input: ReceiptsCreateInput) =>
      assertSuccess(await receiptsCreate(input)),
...
export function useCreateReceiptFull() {
  ...
  return useMutation({
    mutationFn: async (input: ReceiptsCreateFullInput) =>
      assertSuccess(await receiptsCreateFull(input)),
...
export function useUpdateReceipt() {
  ...
  return useMutation({
    mutationFn: async (input: ReceiptsUpdateInput) =>
      assertSuccess(await receiptsUpdate(input)),
    ...
    onSuccess: async (updated) => {
      ...
      if (typeof receiptId === "string" && receiptId) {
        qc.setQueriesData(
          { queryKey: [...receiptsKeys.all, "full", receiptId], exact: false },
          updated,
        );
      }
...
export function useDeleteReceipt() {
  ...
  return useMutation({
    mutationFn: async (input: ReceiptsDeleteInput) =>
      assertSuccess(await receiptsDelete(input)),

    onMutate: async (vars) => {
      const receiptId =
        (vars.body as unknown as { receiptId?: string }).receiptId ?? "";
...
export function useReceiptsAll(
  input?: { query?: StandardListQuery; sort?: Record<string, unknown> },
  opts?: { enabled?: boolean },
) {
  return useQuery({
    queryKey: [
    ...
    queryFn: async () => {
      ...
      const all: ReceiptListItem[] = Array.isArray(firstRes.data)
        ? [...firstRes.data]
        : [];
